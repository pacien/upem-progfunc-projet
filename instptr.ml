...
let instptr_string instptr =
  let string_of_inst = function
    | Zero(a) -> "Zero " ^ (string_of_int a)
    | Succ(a) -> "Succ " ^ (string_of_int a)
    | Copy(a, b) -> "Copy " ^ (string_of_int a) ^ " " ^ (string_of_int b)
    | Jump(a, b, c) -> "Jump " ^ (string_of_int a) ^ " " ^ (string_of_int b) ^ " " ^ (string_of_int c)
